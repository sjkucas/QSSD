...
class NestedTensor(object):
    ...
    def to(self, device, non_blocking=False, dtype=torch.float32):
        torch.Tensor.to
        # type: (Device) -> NestedTensor # noqa
        cast_tensor = self.tensors.to(device, non_blocking=non_blocking, dtype=dtype)
        mask = self.mask
        if mask is not None:
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], fill_value=0):
    ...
    if tensor_list[0].ndim == 3:
        ...
        t = roundup_to_multiple(t, 1024) # 16 (num_frames) * 64 (requirement of multi-scale features)
        dtype = tensor_list[0].dtype
        device = tensor_list[0].device
        
        tensor = torch.full((b, c, t, h, w), dtype=dtype, device=device, fill_value=fill_value)
        mask = torch.ones((b, t), dtype=torch.bool, device=device)
        for video_ft, pad_video_ft, m in zip(tensor_list, tensor, mask):
