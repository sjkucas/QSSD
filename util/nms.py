# Functions for 1D NMS, modified from:
# https://github.com/open-mmlab/mmcv/blob/master/mmcv/ops/nms.py
import torch
import numpy as np
import nms_1d_cpu

def apply_nms(dets, nms_thr=0.7, min_score=0.001, topk=200, voting_thresh=0.7, multi_class=True):
    segs = dets[:, :2]
    scores = dets[:, 2]
    labels = dets[:, 3]

    if multi_class:
        unique_classes = np.unique(labels)
        output_dets = []
        for cls in unique_classes:
            indices = np.where(labels == cls)
            this_segs, this_scores, this_labels = NMSop.apply(
                segs[indices], scores[indices], labels[indices], nms_thr, min_score, topk
            )
            #==================

            if voting_thresh > 0:
                this_segs = seg_voting(
                    this_segs,
                    segs,
                    scores,
                    voting_thresh
                )
            #==================
            this_dets = np.concatenate([this_segs, this_scores[:, None], this_labels[:, None]], axis=1)

            output_dets.append(this_dets)
        output_dets = np.concatenate(output_dets, axis=0)
    else:
        this_segs, this_scores, this_labels = NMSop.apply(
            segs, scores, labels, nms_thr, min_score, topk
        )
        # seg voting
        if voting_thresh > 0:
            this_segs = seg_voting(
                this_segs,
                segs,
                scores,
                voting_thresh
            )
        output_dets = np.concatenate([this_segs, this_scores[:, None], this_labels[:, None]], axis=1)

    sort_idx = output_dets[:, 2].argsort()[::-1]
    output_dets = output_dets[sort_idx, :]
    return output_dets

def apply_softnms(dets, nms_thr=0.7, min_score=0.001, topk=200, sigma=0.5, voting_thresh=0.7, multi_class=True):
    segs = dets[:, :2]
    scores = dets[:, 2]
    labels = dets[:, 3]
    if multi_class:
        unique_classes = np.unique(labels)
        output_dets = []
        for cls in unique_classes:
            indices = np.where(labels == cls)
            this_segs, this_scores, this_labels = SoftNMSop.apply(
                segs[indices], scores[indices], labels[indices], nms_thr, sigma, min_score, 2, topk
            )
            if voting_thresh > 0:
                this_segs = seg_voting(
                    this_segs,
                    segs,
                    scores,
                    voting_thresh
                )
            this_dets = np.concatenate([this_segs, this_scores[:, None], this_labels[:, None]], axis=1)

            output_dets.append(this_dets)
        output_dets = np.concatenate(output_dets, axis=0)
    else:
        this_segs, this_scores, this_labels = SoftNMSop.apply(
            segs, scores, labels, nms_thr, sigma, min_score, 2, topk
        )
        # seg voting
        if voting_thresh > 0:
            this_segs = seg_voting(
                this_segs,
                segs,
                scores,
                voting_thresh
            )
        output_dets = np.concatenate([this_segs, this_scores[:, None], this_labels[:, None]], axis=1)

    sort_idx = output_dets[:, 2].argsort()[::-1]
    output_dets = output_dets[sort_idx, :]
    return output_dets

class NMSop(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, segs, scores, cls_idxs,
        nms_thr, min_score, max_num
    ):
        segs = torch.from_numpy(segs).float()
        scores = torch.from_numpy(scores).float()
        cls_idxs = torch.from_numpy(cls_idxs).float()
        # vanilla nms will not change the score, so we can filter segs first
        is_filtering_by_score = (min_score > 0)
        if is_filtering_by_score:
            valid_mask = scores > min_score
            segs, scores = segs[valid_mask], scores[valid_mask]
            cls_idxs = cls_idxs[valid_mask]
            valid_inds = torch.nonzero(
                valid_mask, as_tuple=False).squeeze(dim=1)

        # nms op; return inds that is sorted by descending order
        inds = nms_1d_cpu.nms(
            segs.contiguous().cpu(),
            scores.contiguous().cpu(),
            iou_threshold=float(nms_thr))
        # cap by max number
        if max_num > 0:
            inds = inds[:min(max_num, len(inds))]
        # return the sorted segs / scores
        sorted_segs = segs[inds]
        sorted_scores = scores[inds]
        sorted_cls_idxs = cls_idxs[inds]
        return sorted_segs.clone().numpy(), sorted_scores.clone().numpy(), sorted_cls_idxs.clone().numpy()


class SoftNMSop(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, segs, scores, cls_idxs,
        nms_thr, sigma, min_score, method, max_num
    ):
        segs = torch.from_numpy(segs).float()
        scores = torch.from_numpy(scores).float()
        cls_idxs = torch.from_numpy(cls_idxs).float()
        # pre allocate memory for sorted results
        dets = segs.new_empty((segs.size(0), 3), device='cpu')
        # softnms op, return dets that stores the sorted segs / scores
        inds = nms_1d_cpu.softnms(
            segs.cpu(),
            scores.cpu(),
            dets.cpu(),
            iou_threshold=float(nms_thr),
            sigma=float(sigma),
            min_score=float(min_score),
            method=int(method))
        # cap by max number
        if max_num > 0:
            n_segs = min(len(inds), max_num)
        else:
            n_segs = len(inds)
        sorted_segs = dets[:n_segs, :2]
        sorted_scores = dets[:n_segs, 2]
        sorted_cls_idxs = cls_idxs[inds]
        sorted_cls_idxs = sorted_cls_idxs[:n_segs]
        return sorted_segs.clone().numpy(), sorted_scores.clone().numpy(), sorted_cls_idxs.clone().numpy()

def gaussian_mapping(x, center, sigma, start, end):
    """生成截断区间的高斯函数，x在[start, end]外为0"""
    gauss = torch.exp(-0.5 * ((x - center) / sigma) ** 2)
    mask = (x >= start) & (x <= end)
    return gauss * mask.float()
def refine_nms_segs_with_weights(nms_segs, all_segs, seg_weights, num_points=1000):
    refined_segs = []

    for i in range(nms_segs.shape[0]):
        # 找到权重大于0的 all_segs 索引
        mask = seg_weights[i] > 0
        related_segs = all_segs[mask]
        weights = seg_weights[i][mask]

        if related_segs.shape[0] == 0:
            refined_segs.append(nms_segs[i].tolist())
            continue

        # 新的绘制区间：所有相关提议的最小start到最大end
        start = related_segs[:, 0].min().item()
        end = related_segs[:, 1].max().item()

        # 构造区间网格
        x = torch.linspace(start, end, num_points)
        gaussians = []

        for seg, w in zip(related_segs, weights):
            s, e = seg
            center = (s + e) / 2
            sigma = (e - s) / 4  # 控制宽度
            gauss = gaussian_mapping(x, center, sigma, s, e)
            gaussians.append(w * gauss)  # 乘以权重

        # 加权混合高斯
        mix_gaussian = torch.stack(gaussians, dim=0).sum(dim=0)

        # 阈值线（中间线）
        threshold = mix_gaussian.max() / 2

        # 找到交点
        above = mix_gaussian >= threshold
        if above.any():
            new_start_idx = torch.argmax(above.int())
            new_end_idx = len(above) - torch.argmax(above.flip(0).int()) - 1
            new_start = x[new_start_idx].item()
            new_end = x[new_end_idx].item()
            refined_segs.append([new_start, new_end])
        else:
            refined_segs.append([start, end])

    return torch.tensor(refined_segs, dtype=nms_segs.dtype)
def seg_voting(nms_segs, all_segs, all_scores, iou_threshold, score_offset=1.5):
    """
        blur localization results by incorporating side segs.
        this is known as bounding box voting in object detection literature.
        slightly boost the performance around iou_threshold
    """

    # *_segs : N_i x 2, all_scores: N,
    # apply offset
    offset_scores = all_scores + score_offset
    nms_segs = torch.from_numpy(nms_segs)
    all_segs = torch.from_numpy(all_segs)
    all_scores = torch.from_numpy(all_scores)


    # computer overlap between nms and all segs
    # construct the distance matrix of # N_nms x # N_all
    num_nms_segs, num_all_segs = nms_segs.shape[0], all_segs.shape[0]
    ex_nms_segs = nms_segs[:, None].expand(num_nms_segs, num_all_segs, 2)
    ex_all_segs = all_segs[None, :].expand(num_nms_segs, num_all_segs, 2)

    # compute intersection
    left = torch.maximum(ex_nms_segs[:, :, 0], ex_all_segs[:, :, 0])
    right = torch.minimum(ex_nms_segs[:, :, 1], ex_all_segs[:, :, 1])
    inter = (right-left).clamp(min=0)

    # lens of all segments
    nms_seg_lens = ex_nms_segs[:, :, 1] - ex_nms_segs[:, :, 0]
    all_seg_lens = ex_all_segs[:, :, 1] - ex_all_segs[:, :, 0]

    # iou
    iou = inter / (nms_seg_lens + all_seg_lens - inter)

    # get neighbors (# N_nms x # N_all) / weights
    seg_weights = (iou >= iou_threshold).to(all_scores.dtype) * all_scores[None, :] * iou

    # refine_segs = refine_nms_segs_with_weights(nms_segs, all_segs, seg_weights)
    # print(refine_segs.shape)

    seg_weights /= torch.sum(seg_weights, dim=1, keepdim=True)
    refined_segs = seg_weights @ all_segs

    return refined_segs.numpy()

def batched_nms(
    segs,
    scores,
    cls_idxs,
    iou_threshold,
    min_score,
    max_seg_num,
    # use_soft_nms=True,
    use_soft_nms=True,
    multiclass=True,
    sigma=0.5,
    voting_thresh=0.75,
):
    # Based on Detectron2 implementation,
    num_segs = segs.shape[0]
    # corner case, no prediction outputs
    if num_segs == 0:
        return torch.zeros([0, 2]),\
               torch.zeros([0,]),\
               torch.zeros([0,], dtype=cls_idxs.dtype)

    if multiclass:
        # multiclass nms: apply nms on each class independently
        new_segs, new_scores, new_cls_idxs = [], [], []
        for class_id in torch.unique(cls_idxs):
            curr_indices = torch.where(cls_idxs == class_id)[0]
            # soft_nms vs nms
            if use_soft_nms:
                sorted_segs, sorted_scores, sorted_cls_idxs = SoftNMSop.apply(
                    segs[curr_indices],
                    scores[curr_indices],
                    cls_idxs[curr_indices],
                    iou_threshold,
                    sigma,
                    min_score,
                    2,
                    max_seg_num
                )
            else:
                sorted_segs, sorted_scores, sorted_cls_idxs = NMSop.apply(
                    segs[curr_indices],
                    scores[curr_indices],
                    cls_idxs[curr_indices],
                    iou_threshold,
                    min_score,
                    max_seg_num
                )
            # disable seg voting for multiclass nms, no sufficient segs

            # fill in the class index
            new_segs.append(sorted_segs)
            new_scores.append(sorted_scores)
            new_cls_idxs.append(sorted_cls_idxs)

        # cat the results
        new_segs = torch.cat(new_segs)
        new_scores = torch.cat(new_scores)
        new_cls_idxs = torch.cat(new_cls_idxs)

    else:
        # class agnostic
        if use_soft_nms:
            new_segs, new_scores, new_cls_idxs = SoftNMSop.apply(
                segs, scores, cls_idxs, iou_threshold,
                sigma, min_score, 2, max_seg_num
            )
        else:
            new_segs, new_scores, new_cls_idxs = NMSop.apply(
                segs, scores, cls_idxs, iou_threshold,
                min_score, max_seg_num
            )
        # seg voting
        if voting_thresh > 0:
            new_segs = seg_voting(
                new_segs,
                segs,
                scores,
                voting_thresh
            )

    # sort based on scores and return
    # truncate the results based on max_seg_num
    _, idxs = new_scores.sort(descending=True)
    max_seg_num = min(max_seg_num, new_segs.shape[0])
    # needed for multiclass NMS
    new_segs = new_segs[idxs[:max_seg_num]]
    new_scores = new_scores[idxs[:max_seg_num]]
    new_cls_idxs = new_cls_idxs[idxs[:max_seg_num]]

    return new_segs, new_scores, new_cls_idxs
