...
class TADEvaluator(object):
    ...
    def update(self, pred):
        ...
        for k, v in pred_numpy.items():
            ...
            for nms_mode in self.nms_mode:
                ...
                # dets = input_dets
                if nms_mode == 'nms':
                    dets = apply_nms(dets, nms_thr=0.8, min_score=0.3, topk=self.topk, voting_thresh=self.voting_thresh, multi_class=self.nms_multi_class)
                if nms_mode == 'soft_nms':
                    dets = apply_softnms(dets, nms_thr=self.nms_thr, min_score=self.min_score, topk=self.topk, sigma=self.nms_sigma, voting_thresh=self.voting_thresh, multi_class=self.nms_multi_class)
    ...
    def compute_map(self, nms_mode):
        ...
        with concurrent.futures.ProcessPoolExecutor(min(1, os.cpu_count())) as p:
